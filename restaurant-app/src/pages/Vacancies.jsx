...
import { getVacancies, addVacancy, updateVacancy, deleteVacancy } from '../api/index';
...
const Vacancies = () => {
    ...
    const [newVacancy, setnewVacancy] = useState({ text: "" });
    ...
    const handleActionComplete = async (args) => {
        if (args.requestType === 'save') {
            try {
                await updateVacancy(args.data.id, args.data);
                setErrorMessage("");
                fetchVacancies();
            } catch {
    ...
    const handleActionBegin = async (args) => {
        if (args.requestType === 'delete') {
            try {
                await deleteVacancy(args.data[0].id);
                setErrorMessage("");
                fetchVacancies();
            } catch {
    ...
    return (
        <div className="m-2 md:m-10 mt-24 p-2 md:p-10 bg-white rounded-3xl">
            <Header category="Управління персоналом" title="Вакансії" />
            ...
            >
                <ColumnsDirective>
                    ...
                    <ColumnDirective field='text' headerText='Текст вакансії' width={120} />
                </ColumnsDirective>
